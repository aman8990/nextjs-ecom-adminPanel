...
function AcceptedOrdersData() {
  const { data, error, isLoading, mutate } = useAcceptedOrders();
  ...
  return (
    <div className="mb-32">
      <h1 className="text-center text-4xl my-10">Accepted Orders</h1>
      ...
      <div ref={printRef}>
        {data.map((order) => (
          <div key={order.id} className="order-container">
            <AcceptedOrder order={order} data={data} mutate={mutate} />
          </div>
        ))}
      </div>
    </div>
